        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8 lg:p-10 space-y-8">
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div className="space-y-1">
                    <h1 className="text-3xl md:text-3xl font-extrabold text-slate-900 tracking-tight">Dashboard Admin</h1>
                    <p className="text-slate-500 font-medium">
                        Pantauan kehadiran hari ini, <span className="text-purple-700 font-semibold">{new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    </p>
                </div>
                <div className="flex gap-2">
                    <a href="/admin/reports/attendance" className="px-4 py-2 bg-white text-purple-700 text-sm font-bold border border-purple-100 rounded-lg shadow-sm hover:bg-purple-50 transition-colors flex items-center gap-2">
            ...
            {loadingToday ? (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[1, 2, 3, 4].map(i => <div key={i} className="h-[120px] bg-slate-100 rounded-3xl animate-pulse"></div>)}
                </div>
            ) : errorToday ? (
                ...
                <div className="space-y-8">
                    ...
                    <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            { label: 'Total Guru', val: todaySummary.totalTeachers, color: 'text-purple-700', bg: 'bg-purple-100', icon: <Icons.Users className="w-6 h-6" /> },
                            { label: 'Hadir', val: todaySummary.presentCount, color: 'text-emerald-700', bg: 'bg-emerald-100', icon: <Icons.CheckCircle className="w-6 h-6" /> },
                            { label: 'Terlambat', val: todaySummary.lateCount, color: 'text-amber-700', bg: 'bg-amber-100', icon: <Icons.AlertCircle className="w-6 h-6" /> },
                            { label: 'Tidak Hadir', val: todaySummary.absentCount + todaySummary.leaveCount + todaySummary.sickCount, color: 'text-rose-700', bg: 'bg-rose-100', icon: <Icons.X className="w-6 h-6" /> },
                        ].map((item, idx) => (
                            <div key={idx} className="bg-white p-6 rounded-3xl shadow-lg border border-purple-50 flex items-center gap-4 hover:translate-y-[-2px] transition-transform duration-300">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="bg-white rounded-3xl shadow-lg p-8 border border-purple-50 xl:col-span-2">
                    ...
                    {loadingToday ? (
                        ...
                        <div className="space-y-4">
                            {todaySummary.topLateToday.map((teacher) => (
                                <div key={teacher.teacherId} className="flex items-center justify-between p-4 bg-amber-50 rounded-2xl border border-amber-100">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-amber-200 flex items-center justify-center text-amber-700 font-bold text-lg">
                                            !
                                        </div>
                                        <div>
                                            <div className="font-bold text-slate-800">{teacher.teacherName}</div>
                                            <div className="text-xs text-slate-500 font-mono">{teacher.subject} • NIP: {teacher.nip}</div>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-lg font-bold text-amber-600">{teacher.lateMinutes}m</div>
                                        <div className="text-[10px] font-bold text-slate-400 uppercase">Keterlambatan</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p className="text-slate-800 font-bold mb-1">Semua Datang Tepat Waktu</p>
                ...
                <div className="bg-white rounded-3xl shadow-lg p-8 border border-purple-50 flex flex-col">
                    ...
                    {loadingTopLate ? (
                        ...
                        <div className="flex-1 flex items-center justify-center">
                            <p className="text-red-500 text-sm">{errorTopLate}</p>
                        </div>
                    ) : (topLateMonthly?.data && topLateMonthly.data.length > 0) ? (
                        <div className="space-y-4 flex-1">
                            {topLateMonthly.data.map((teacher, index) => (
                                <div key={teacher.teacherId} className="p-4 rounded-2xl border border-slate-100 bg-slate-50 relative overflow-hidden group hover:border-rose-200 hover:bg-rose-50/30 transition-colors">
                                    <div className="absolute top-0 right-0 bg-slate-200 text-slate-500 text-[10px] font-bold px-2 py-1 rounded-bl-lg group-hover:bg-rose-100 group-hover:text-rose-600 transition-colors">
                                        RANK #{index + 1}
                                    </div>

                                    <div className="mb-2">
                                        <div className="font-bold text-slate-800 leading-tight">{teacher.teacherName}</div>
                                    ...
                                    <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-200/50">
                                        <div>
                                            <div className="text-xs font-bold text-slate-400 uppercase">Total Menit</div>
                                            <div className="text-lg font-bold text-rose-600">{teacher.totalLateMinutes}m</div>
                                        </div>
                                        <div className="w-px h-8 bg-slate-200"></div>
                                        <div>
                                            <div className="text-xs font-bold text-slate-400 uppercase">Frekuensi</div>
                                            <div className="text-lg font-bold text-slate-700">{teacher.lateCount}x</div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
                            <Icons.CheckCircle className="w-12 h-12 text-emerald-300 mb-3" />
                            <p className="text-slate-600 font-medium text-sm">Belum ada keterlambatan bulan ini.</p>
                        </div>
                    )}

                    <div className="mt-6 pt-6 border-t border-slate-100">
                        <a href="/admin/reports/attendance" className="block w-full text-center text-sm font-bold text-purple-600 hover:text-purple-700 hover:underline">
